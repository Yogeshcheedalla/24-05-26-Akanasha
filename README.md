...
Akansha separates everyday conversation from heavier cognitive workflows so the
normal chat and voice experience stays responsive.
...
the default. Research, Agent, and Skill are explicit slow-lane modes for work
that needs deeper reasoning, external providers, live sources, or generated
artifacts.
...
Background memory extraction is bounded: deterministic memory capture runs
